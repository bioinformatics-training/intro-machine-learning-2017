...
```{r echo=T}
geneindex <- 36
D <- read.csv(file = "/Users/christopher_penfold/Desktop/MLCourse/intro-machine-learning/data/Arabidopsis/Arabidopsis_Botrytis_transpose_2.csv", header = TRUE, sep = ",", row.names=1)
...
```{r echo=T}
covSEn <- function(X1,X2,l=1,sig=1,sigman=1) {
  K <- matrix(rep(0, length(X1)*length(X2)), nrow=length(X1))
  for (i in 1:nrow(K)) {
    for (j in 1:ncol(K)) {
...
So there's some slight evidence for model 1 (DE) over model 2 (NDE).
