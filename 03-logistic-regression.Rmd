...
Supervised learning typically involves a component of regression or a component of classification. Regression aims to identify how a particular (continuous) variable of interest, $\mathbf{y}$, is influenced by a set of explanatory variables, $\mathbf{X}$. This involves identifying a function that predicts $y$ from $X$ (trends), which, of course, allows prediction of $y*$ at a new input where there has been no observation $X*$ (forecasting). LINK BACK TO LINEAR MODELS. Classificaiton algorithms, on the other hand, deal with discrete (categorical) valued outpus. A notable example would be where the output is either $0$ or $1$ (logistic). Other categorical values could be when the data falls in to one of three categories "group 1", "group 2" or "group 3". The aim here, is to learn how different input values of $X$ map to a particular output (category), and ultimately to assign categories to a new set of observations, $X*$, whose response variable is unknown.
...
```{r echo=T}
lrfit3 <- train(y~poly(x,degree=2), data=data.frame(x=D[1:24,3],y=D[1:24,geneindex]), method = "lm")
```
...
where $m = [m_1,\ldots,m_n,c]$ are the free parameters. Within R we can infer more complex polynomials to the data using the lm package. In the example below we fit a 3rd order polynomial:
 
```{r echo=T}
lrfit3 <- lm(y~poly(x,degree=3), data=data.frame(x=D[1:24,1],y=D[1:24,geneindex]))
```
 
We can do this within caret:
...
Excercise 3.4. Compare the fits for various polynomial models. What about the RMSE? Which model seems to be best?
...
CAN OPTIMISE DIRECTLY. Derivative

$dL/dK dK/d\theta$

Excercise 3.6: Try fitting plotting the GP for the optimised values of the hyperparmeters. 

Excercise 3.7: Now try fitting a Gaussian process to one of the gene expression profiles in the Botrytis dataset. Hint: Normalise the time axis. You will also re-implement the covSE covariance function and the ML calculation function to account for independent noise.

### Model Selection

As well as a criterior for selection of hyperparameters, the marginal likelihood can additionally be used as a basis for selecting models. For example, we might be interested in comparing how well two different covariance functions fit the data.
...
Excercise 3.8 (optional): Write a function for determining differential expression for two genes. Hint: we are interested in comparing two models, and using Bayes Factor to determine if the genes are DE. In the first mode ... one we assume no differential expression i.e., ...

Excercise 3.9 (optional): Apply the model ...


### Advanced Application: Differential expression analysis as model selection 

Nonlinear regression represent useful tools for modelling. In the previous section we have seen how GPs can also be used to model if two time series are differentially expresssed. Further to this, they can be used to model when the time series diverge. Notable examples include the work by Stegle et al. (2010), Rattray *et al.* (2016), and Penfold, Sybirna *et al.* (2017).

Code for using GPs to infer timing are available from [DEtime](https://github.com/ManchesterBioinference/DEtime) (Rattray *et al.* 2016).
...
Within DEtime, we can call the function \texttt{DEtime_rank} to calculate marginal likelihoods for two time series, similar to our applicaiton in the previous section. Within toptimising the hyperparameters. 
...
For genes that are DE, we can run the code to identify the timing of divergence by calling the function \texttt{DEtime_infer}.
...
Classification algorithms are a supervised learning techniques that assign data to categorical outputs. For example we may have a continuous input variable, $x$, and want to learn how that variable maps to a discrete valued output such as a discrete phenotype. 
...
$\Phi$ is the CDF of normal. Latent variable:

$y* = X^\top \beta + \varepsilon$

where $\varepsilon$ is a GRV.
...
$\ln \mathcal{L} (\beta) = \sum_{i=1}^{n} y_i \ln \Phi (x_i^\prime \beta) +  (1-y_i) \ln \Phi (x_i^\prime \beta)$

can use gradient methods. 

Within logistic regression we have:

$y* = \beta_0 + \beta x + \varepsilon$

Within caret, probit and logistic 

```{r echo=F}
library(pROC)
library(ROCR)

Dpred <- read.csv(file = "/Users/christopher_penfold/Desktop/MLCourse/intro-machine-learning/data/Arabidopsis/Arabidopsis_Botrytis_pred_transpose_3.csv", header = TRUE, sep = ",", row.names=1)

mod_fit2 <- train(y ~ ., data=data.frame(x = D[,3:ncol(D)], y = as.factor(D$Class)), method="glm", family="binomial")
...
We therefore show good ability to predict the state of infection with expression of various genes. 

## GP classification

Bayesian nonparametric approaches to classification exist, and again come with the added advantage that they have a treatment for uncertainity in the inference procedure. In particular we can perofrm classificaiton using GPs. Unlike Gaussian process regression, Gaussian process classification is not analytically tractible and we must instead use approximations. 
GPCs have been implemented in \texttt{caret}. Here we have a choice of including a polynomial kernel, implemented via:
...
mod_fit2 <- train(y ~ x, data=data.frame(x = D$AT1G03960, y = as.factor(D$Class)), method="cforest")

mod_fit2 <- train(y ~ x, data=data.frame(x = D$AT1G03960, y = as.factor(D$Class)), method="svmRadialWeights")

mod_fit2 <- train(y ~ x, data=data.frame(x = D$AT1G03960, y = as.factor(D$Class)), method="plr")

mod_fit2 <- train(y ~ x, data=data.frame(x = D$AT1G03960, y = as.factor(D$Class)), method="multinom")
